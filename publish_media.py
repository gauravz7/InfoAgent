#!/usr/bin/env python3
"""Publish generated media-digest issues into a GitHub Pages /media-digest/ site.

The self-contained twin of ``publish.py`` for the weekly Generative Media digest.
Takes an issue produced by ``app_media.runner`` (output_media/<date>/) and writes
it into <site>/media-digest/ (a SEPARATE section from the daily /digest/ site):

  media-digest/<date>/index.html   web issue page (relative image links)
  media-digest/<date>/images/...   the diagrams
  media-digest/index.html          archive index + RSS link (no email signup yet)
  media-digest/rss.xml             RSS 2.0; <content:encoded> is email-safe HTML
                                   (absolute image URLs)

Usage:
  python3 publish_media.py --date 2026-07-26 --site /path/to/gauravz7.github.io \
      --base-url https://gauravz7.github.io/media-digest
"""
...
from app_media import config
from app_media.render import DISCLAIMER_TEXT

SECTION = "media-digest"
FEED_TITLE = "Generative Media Weekly Digest"
FEED_DESC = ("Top generative-media research papers + the week's media news "
             "(image, video, audio & music generation) — image-forward, cited, "
             "and fact-checked. Weekly.")
MAX_RSS_ITEMS = 20
...
def _issue_title(issue_html: str, date: str) -> str:
    ...
    return f"Generative Media · {date}" + (f" — {lead}" if lead else "")
...
def publish_issue(output_root: str, date: str, site: str, base_url: str) -> dict:
    """Copy one generated issue into <site>/media-digest/<date>/. Returns issue meta."""
    ...
    dest_dir = os.path.join(site, SECTION, date)
    ...
    print(f"  [publish] issue -> {SECTION}/{date}/ ({len(os.listdir(src_images)) if os.path.isdir(src_images) else 0} images)")
...
def rebuild_index(site: str, base_url: str) -> None:
    """Regenerate media-digest/index.html: archive list + RSS box (pink theme).

    No email signup yet, so (unlike ``publish.py``) there is no MailerLite form —
    just an RSS subscribe link."""
    t = config.THEME
    rows = []
    for date in _list_issue_dates(site):
        issue_html = _read(os.path.join(site, SECTION, date, "index.html"))
...
<style>
  ...
  .sub a.rsslink{{display:inline-block;background:{t['accent']};color:#fff;border:0;
    border-radius:8px;padding:.7rem 1.4rem;font-size:1rem;font-weight:700;
    text-decoration:none;cursor:pointer}}
  .sub a.rsslink:hover{{opacity:.9}}
</style>
</head>
<body><div class="wrap">
  ...
  <div class="sub">
    <h3>Follow the Generative Media Weekly Digest</h3>
    <p style="margin:.2em 0 .9rem;color:{t['text_soft']};font-size:.9rem">
      Top arXiv generative-media papers + the week's media news, published every
      Sunday. Subscribe by feed:</p>
    <a class="rsslink" href="{base_url}/rss.xml">RSS feed</a>
  </div>
  ...
  <footer>
    ...
    _write(os.path.join(site, SECTION, "index.html"), page)
    print(f"  [publish] index -> {SECTION}/index.html ({len(_list_issue_dates(site))} issues)")


DIGEST_CARD_START = "<!-- MEDIA-DIGEST-CARD:START -->"
DIGEST_CARD_END = "<!-- MEDIA-DIGEST-CARD:END -->"
...
def _digest_card_html(date: str, base_url: str) -> str:
    ...
    return (
        f'{DIGEST_CARD_START}\n'
        '        <article class="card">\n'
        '          <span class="tag">Weekly · AI-generated · auto-published</span>\n'
        f'          <h2><a class="stretch" href="{base_url}/{date}/">Generative Media Digest — {date}</a></h2>\n'
        '          <p>This week&#x27;s issue — the top generative-media research '
        'papers (image, video, audio &amp; music generation and editing), the '
        'week&#x27;s media news, and practical engineering blogs — freshly generated '
        'and published. A new digest lands here every week, with older issues '
        'archived. <em>All text and images are AI-generated and may contain '
        'mistakes.</em></p>\n'
...
def rebuild_rss(site: str, base_url: str, now: _dt.datetime = None) -> None:
    """Regenerate media-digest/rss.xml with content:encoded (absolute-image) HTML."""
...
{chr(10).join(items_xml)}
    ...
    _write(os.path.join(site, SECTION, "rss.xml"), rss)
    print(f"  [publish] rss   -> {SECTION}/rss.xml ({len(items_xml)} items)")


def main(argv=None):
    ap = argparse.ArgumentParser(description="Publish a media-digest issue into a Pages /media-digest/ site")
    ...
    ap.add_argument("--base-url", default="https://gauravz7.github.io/media-digest",
                    help="public base URL of the media-digest section")
    ap.add_argument("--output-root", default=config.OUTPUT_ROOT,
                    help="where app_media.runner wrote the issue")
    ...
    print(f"Published {args.date} into {os.path.join(args.site, SECTION)}")
