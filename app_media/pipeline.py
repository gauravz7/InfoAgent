...
def fetch_upcoming_events(n: int = None, verbose: bool = True) -> List[dict]:
    ...
    tool = types.Tool(google_search=types.GoogleSearch())
    prompt = (
        f"Search the web for the {n} most important UPCOMING {config.EVENT_FOCUS} "
        "events happening in the FUTURE (from today onward), over roughly the next "
        "6 months. Only include events whose START date is in the future. Rank by "
        "prominence and how soon they occur. For each give: the exact event NAME; "
        "the START date (YYYY-MM-DD) and the END date (YYYY-MM-DD, same as start if "
        "single-day); the LOCATION (city, country — or 'Virtual'); the official "
        "event URL (a real https link to the event site); and a one-sentence note "
        "on why it matters / what to expect.\n\n"
        'Return STRICT JSON: array of {"name":str,"date":str,"end_date":str,'
        '"location":str,"url":str,"why":str}, sorted by date ascending.'
    )
    grounding_urls = []
    try:
        resp = _api(lambda: get_client().models.generate_content(
            ...
            label="events-search", search=True)
        items = _loads(resp.text or "")
        grounding_urls = _grounding_urls(resp)
    except Exception as exc:  # noqa: BLE001
        if verbose:
            print(f"  [events] grounded fetch failed ({exc})")
        items = []
    ...
    for it in (items or []):
        if not isinstance(it, dict) or not str(it.get("name", "")).strip():
            continue
        date = str(it.get("date", "")).strip()[:10]
        try:  # drop events clearly in the past; keep undated/malformed ones
            if date and _dt.date.fromisoformat(date) < today:
                continue
        except ValueError:
            pass
        clean.append({
            ...
            "date": date,
            "end_date": str(it.get("end_date", "")).strip()[:10],
    ...
    for c in clean:
        if not c["url"].startswith("http"):
            c["url"] = next(gi, "")
    clean.sort(key=lambda c: c["date"] or "9999-12-31")
    clean = clean[:n]
    if verbose:
        n_url = sum(1 for c in clean if c["url"].startswith("http"))
        print(f"  [events] fetched {len(clean)} upcoming events ({n_url} with links)")
    return clean
