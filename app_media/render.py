...
def _css() -> str:
    ...
    * {{ box-sizing:border-box; }}
    ...
    .events {{ background:#fff; border:1px solid var(--rule); border-left:4px solid var(--accent);
      padding:16px 22px; margin:28px 0; border-radius:6px; }}
    .events h3 {{ margin:.2em 0 .6em; font-size:14px; text-transform:uppercase;
      letter-spacing:.08em; color:var(--accent); }}
    .events ul {{ margin:0; padding-left:18px; }} .events li {{ margin:.55em 0; }}
    .events .ev-name {{ font-weight:700; }}
    .events .ev-name a {{ color:var(--accent); text-decoration:none; }}
    .events .ev-meta {{ display:block; color:var(--text-soft); font-size:13.5px; }}
    .events .ev-why {{ display:block; color:var(--text); font-size:14px; margin-top:2px; }}
    .footer {{ margin-top:56px; padding-top:20px; border-top:3px solid var(--accent);
      color:var(--text-soft); font-size:13px; }}
    """


def _event_banner(events: List[dict]) -> str:
    """A compact 'where the next big AI event is happening' box (name/date/loc/link)."""
    if not events:
        return ""
    rows = []
    for e in events:
        name = html.escape(e.get("name", "") or "Event")
        start = e.get("date", "") or ""
        end = e.get("end_date", "") or ""
        when = html.escape(start)
        if end and end != start:
            when = f"{html.escape(start)} – {html.escape(end)}"
        loc = html.escape(e.get("location", "") or "")
        why = html.escape(e.get("why", "") or "")
        url = e.get("url", "") or ""
        meta = " · ".join(x for x in (when, loc) if x)
        head = (f'<a href="{html.escape(url, quote=True)}">{name}</a>'
                if url.startswith("http") else name)
        line = f'<li><span class="ev-name">{head}</span>'
        if meta:
            line += f'<span class="ev-meta">{meta}</span>'
        if why:
            line += f'<span class="ev-why">{why}</span>'
        rows.append(line + "</li>")
    return (
        '<aside class="events" aria-label="Upcoming AI events">'
        '<h3>📅 Where the next big AI event is happening</h3>'
        f'<ul>{"".join(rows)}</ul></aside>'
    )
...
def build_full_html(title: str, subtitle: str, date_str: str,
                    ...
                    blog_sections: List[dict] = None, events: List[dict] = None) -> str:
    """Assemble the full digest: Top papers + (optional) AI-news + (optional) blogs.

    `events` (optional) renders a compact 'next big AI event' box near the top.
    """
